...
export const projectContentTranslations: Record<string, Record<TranslatedLocale, ProjectContent>> = {
  "southeast-asia-colocation-expansion": {
    ar: {
      ...
      solution: "وفّرت هايسِن إطارات UPS معيارية مُهيّأة للتشغيل بنمط N+1، ووحدات PDU ذكية لقياس استهلاك المستأجرين على مستوى المنفذ، ومواصفة خزانة مركز بيانات موحدة نُشرت عبر القاعتين، مع خطة تركيب على مراحل مرتبة حول حمل المستأجرين الحالي.",
    ...
    fr: {
      ...
      solution: "Haisen a fourni des châssis d'onduleur modulaires configurés pour un fonctionnement N+1, des PDU intelligents pour le comptage des locataires au niveau de la prise, et une spécification d'armoire de centre de données standardisée déployée dans les deux salles, avec un plan d'installation par phases séquencé autour de la charge des locataires existants.",
    ...
    ru: {
      ...
      solution: "Haisen поставила модульные шасси ИБП, настроенные для работы N+1, интеллектуальные PDU для учёта арендаторов на уровне розетки, и стандартизированную спецификацию шкафа ЦОД, развёрнутую в обоих залах, с поэтапным планом установки, выстроенным вокруг существующей нагрузки арендаторов.",
    ...
    zh: {
      ...
      solution: "海森提供了配置为N+1运行的模块化UPS机架、支持插座级租户计量的智能PDU,以及在两个机房统一部署的数据中心机柜规格,并制定了围绕现有租户负载排布的分阶段安装计划。",
  ...
  "middle-east-telecom-network-rollout": {
    ar: {
      ...
      solution: "وفّرت هايسِن وحدات ATS PDU للتحويل التلقائي للتغذية وقالب خزانة شبكة موحد، ما مكّن فرق التركيب لدى المشغل من تكرار نفس تصميم غرفة المعدات في كل موقع جديد.",
    ...
    fr: {
      ...
      solution: "Haisen a fourni des PDU ATS pour la commutation automatique d'alimentation et un modèle d'armoire réseau standardisé, permettant aux équipes d'installation de l'opérateur de reproduire la même conception de local technique sur chaque nouveau site.",
    ...
    es: {
      ...
      solution: "Haisen suministró PDU ATS para la conmutación automática de alimentación y una plantilla de armario de red estandarizada, permitiendo que los equipos de instalación del operador replicaran el mismo diseño de sala de equipos en cada nuevo sitio.",
    ...
    ru: {
      ...
      solution: "Haisen поставила ATS PDU для автоматического переключения ввода и стандартизированный шаблон сетевого шкафа, что позволило монтажным бригадам оператора тиражировать один и тот же дизайн технического помещения на каждом новом объекте.",
    ...
    zh: {
      ...
      solution: "海森提供了用于自动电源切换的ATS PDU及标准化网络机柜模板,使运营商的安装团队能够在每个新站点复制相同的设备间设计。",
  ...
  "west-africa-edge-container-deployment": {
    ar: {
      ...
      solution: "سلّمت هايسِن مركز بيانات بحاوية 40 قدماً متكاملاً بالكامل من المصنع مع توزيع طاقة PDU ذكي وUPS معياري، وتبريد دقيق، وإخماد حريق، ومراقبة عن بُعد، اختُبر في المصنع قبل الشحن.",
      ...
      implementation: "تم تكامل الحاوية واختبار وظائفها بالكامل في مصنع هايسِن، وشُحنت عبر لوجستيات الحاويات القياسية، وشُغّلت في الموقع بعد تحضير الأساس وتوصيل المرافق.",
    ...
    fr: {
      ...
      solution: "Haisen a livré un data center en conteneur de 40 pieds entièrement pré-intégré en usine avec distribution électrique PDU intelligent et onduleur modulaire, refroidissement de précision, extinction incendie et surveillance à distance, testé à l'usine avant expédition.",
      ...
      implementation: "Le conteneur a été entièrement intégré et testé fonctionnellement à l'usine Haisen, expédié par logistique de conteneur standard, et mis en service sur site après préparation des fondations et raccordement aux réseaux.",
    ...
    es: {
      ...
      solution: "Haisen entregó un centro de datos en contenedor de 40 pies totalmente preintegrado en fábrica con distribución de energía mediante PDU inteligente y SAI modular, refrigeración de precisión, extinción de incendios y monitoreo remoto, probado en fábrica antes del envío.",
      ...
      implementation: "El contenedor fue completamente integrado y probado funcionalmente en la fábrica de Haisen, enviado mediante logística estándar de contenedores, y puesto en marcha en el sitio tras la preparación de cimientos y la conexión de servicios.",
    ...
    ru: {
      ...
      solution: "Haisen поставила полностью интегрированный на заводе 40-футовый контейнерный ЦОД с распределением питания через интеллектуальный PDU и модульный ИБП, прецизионным охлаждением, пожаротушением и удалённым мониторингом, протестированный на заводе перед отгрузкой.",
      ...
      implementation: "Контейнер был полностью интегрирован и функционально протестирован на заводе Haisen, отгружен стандартной контейнерной логистикой и введён в эксплуатацию на объекте после подготовки фундамента и подключения коммуникаций.",
    ...
    zh: {
      ...
      solution: "海森交付了一套完全出厂集成的40英尺集装箱数据中心,配备智能PDU及模块化UPS配电、精密制冷、消防及远程监控系统,出厂前已完成测试。",
      ...
      implementation: "该集装箱在海森工厂完成全面集成及功能测试,通过标准集装箱物流运输,并在基础准备及市政接入完成后于现场完成调试。",
